...
class CalendarSyncClientV2Test: XCTestCase {
  ...
  @MainActor func testUnsyncRemovedCalendar() async throws {
    ...
    // Allow for async notification setup.
    await Task.yield()
    ...
    let notificationExpectation = XCTestExpectation(description: "EventStore Notification")
    NotificationCenter.default.addObserver(
      forName: eventStore.observingEventName,
      object: nil,
      queue: .main
    ) { _ in
    ...
    // This await is dual purpose. It yields time for the notification to get processed async, and
    // it fulfills the event store notification expectation.
    await fulfillment(of: [notificationExpectation], timeout: 0.001)
    ...
    if mockChannel.writtenMessages.count == 3 {
